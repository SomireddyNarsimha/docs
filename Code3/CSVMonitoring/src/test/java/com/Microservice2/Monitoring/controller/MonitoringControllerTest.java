...
class MonitoringControllerTest {
    ...
    void testGetLatestRecord_Success() {
        ...
        Records mockRecord = new Records(1L, LocalDateTime.now().toString(), 7.0, 0.5, 10.0, 20.0, 30.0);

        // Simulate service returning a record
        when(monitorService.getLatest()).thenReturn(mockRecord);

        // Call the controller method
        ResponseEntity<Records> response = monitoringController.getLatest();
        ...
        verify(monitorService, times(1)).getLatest();
    }
    ...
    void testGetLatestRecord_NotFound() {
        ...
        when(monitorService.getLatest()).thenReturn(null);
